...
void set_loop_counter(AST_Node *node){
	...
	if( strcmp(incr_node->entry->st_name, assign_node->entry->st_name) ){
		fprintf(stderr, "La variable utilizada en init y en incr de for no es la misma\n");
		exit(1);
	}	
...
int expression_data_type(AST_Node *node){
	...
	switch(node->type){
		...
		default: /* wrong choice case */
			fprintf(stderr, "Error en la selección de nodos\n");
			exit(1);
	}
}
	...
	switch(node->type){
		...
		case CONST_NODE:
			...
			switch(temp_const->const_type){
				case INT_TYPE:
					printf("Se le trato de asginar: %d\n", temp_const->val.ival);
					break;
				case REAL_TYPE:
					printf("Se le trato de asignar un tipo int %.2f\n", temp_const->val.fval);
					break;
				case CHAR_TYPE:
					printf("%c\n",  temp_const->val.cval);
					break;
